        ...
        if fromItem then
            ...
            if not RSGCore.Functions.HasPermission(src, 'admin') then
                ...
                if fromTooFar or toTooFar then
                    ...
                    local message = fromTooFar and 'Source inventory is too far from you.' or
                        'Target inventory is too far from you.'
                    ...
                    if toId ~= fromId then
                        ...
                        if addSuccessFrom and addSuccessTo then
                            if Inventory.RemoveItem(fromId, fromItem.name, fromItemAmount, fromSlot, 'swapped item', isMove) and
                                ...
                                Inventory.AddItem(toId, fromItem.name, fromItemAmount, toSlot, fromItem.info,
                                    'swapped item')
                                Inventory.AddItem(fromId, toItem.name, toItemAmount, fromSlot, toItem.info,
                                    'swapped item')
                            end
                        end
                    else
